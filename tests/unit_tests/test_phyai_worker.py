...
@pytest.mark.asyncio
async def test_actor_bootstrap_is_full_then_returns_to_selective_sync():
    class _SenderSyncer:
        def __init__(self) -> None:
    ...
    actor.param_names_need_sync = ["model.trainable"]
    actor.cfg = OmegaConf.create({"rollout": {"rollout_backend": "phyai"}})
    ...
    actor.get_rollout_sync_version = lambda: 3
    ...
    assert actor.weight_syncer.init_calls == [
        ["model.trainable", "model.frozen"],
        ["model.trainable"],
    ]
    assert actor.weight_syncer.sync_calls == [
        (["model.trainable", "model.frozen"], 3),
        (["model.trainable"], 3),
    ]


def test_first_rollout_sync_bypasses_periodic_cadence():
    class _Handle:
        def wait(self):
            return None

    runner = object.__new__(EmbodiedRunner)
    runner.weight_sync_interval = 4
    runner._rollout_weights_initialized = False
    runner.rollout = SimpleNamespace(sync_model_from_actor=lambda: _Handle())
    runner.actor = SimpleNamespace(sync_model_to_rollout=lambda: _Handle())

    assert runner._should_sync_rollout_weights(step=3)
    runner.update_rollout_weights()
    assert not runner._should_sync_rollout_weights(step=3)
    assert runner._should_sync_rollout_weights(step=4)
...
def test_init_worker_uses_model_cfg_and_fp32_vision_boundary(monkeypatch):
    captured = {}
    ...
    class _TransformRunner:
        def __init__(self, input_transforms, output_transforms, **kwargs):
            captured["transforms"] = (input_transforms, output_transforms, kwargs)
    ...
    monkeypatch.setattr(
        ...
        lambda _cfg: ("pi05_libero", ["input"], ["output"]),
    )
    monkeypatch.setattr(phyai_worker, "OpenPITransformRunner", _TransformRunner)
    ...
    assert plugin_args.defer_scheduler_setup is False
    assert plugin_cfg.chunk_size == 10
    assert plugin_cfg.max_action_dim == 32
    assert plugin_cfg.num_inference_steps == 3
    assert plugin_cfg.action_chunk == 5
    assert plugin_cfg.add_value_head
    assert plugin_cfg.detach_critic_input
    assert plugin_cfg.value_after_vlm
    assert captured["transforms"][2]["device"] == torch.device("cpu")


def test_phyai_preprocessing_matches_hf_wrapper(monkeypatch):
    ...
    hf = OpenPiPytorchEvalActionModel(
        torch.nn.Linear(1, 1),
        num_steps=3,
        action_env_dim=7,
        action_chunk=5,
    ...
    hf.setup_wrappers([input_transform], [output_transform])
    worker = object.__new__(PhyAIWorker)
    worker._engine_device = torch.device("cpu")
    worker._num_images = 2
    worker._transform_runner = OpenPITransformRunner(
        [input_transform],
        [output_transform],
        config_name="pi05_libero",
        state_indices=None,
    ...
    hf_observation = hf._observation_dict_to_device(
        hf.input_transform(hf._repack_env_obs(env_obs))
    )
    prepared_hf = preprocess_observation(hf_observation, train=False)

    assert torch.equal(request.input_ids, hf_observation.tokenized_prompt)
    assert torch.equal(
        request.lang_lens, hf_observation.tokenized_prompt_mask.sum(dim=-1)
    )
    assert torch.equal(replay.state, hf_observation.state)
    for index, key in enumerate(IMAGE_KEYS[:2]):
        assert torch.equal(replay.images[key], prepared_hf.images[key])
        assert torch.equal(
            request.pixel_values[:, index],
            prepared_hf.images[key].permute(0, 3, 1, 2),
        )
    assert request.pixel_values.dtype is torch.float32


def test_hf_preprocessing_tracks_model_device_after_setup(monkeypatch):
    openpi = ModuleType("openpi")
    transforms = ModuleType("openpi.transforms")
    transforms.compose = lambda functions: functions[0]
    openpi.transforms = transforms
    ...
    model.setup_wrappers([lambda sample: sample], [lambda sample: sample])
    model.model.to("meta")
    observation = model._observation_dict_to_device(
        {
            "image": {IMAGE_KEYS[0]: torch.zeros(1, 2, 2, 3)},
            "image_mask": {IMAGE_KEYS[0]: torch.ones(1, dtype=torch.bool)},
            "state": torch.zeros(1, 8, dtype=torch.float64),
            "tokenized_prompt": torch.zeros(1, 4, dtype=torch.long),
            "tokenized_prompt_mask": torch.ones(1, 4, dtype=torch.bool),
        }
    )

    assert observation.images[IMAGE_KEYS[0]].device.type == "meta"
    assert observation.state.device.type == "meta"
    assert observation.state.dtype is torch.float32
...
def test_training_predict_returns_actor_replay_contract(monkeypatch):
    ...
    class _TransformRunner:
        def repack_env_obs(self, env_obs):
            return env_obs

        def input_transform(self, _env_obs):
            return {
        ...
        def observation_to_device(self, _transformed):
            return processed

        def output_transform(self, output):
            return {"actions": output["actions"][:, :5, :7]}
    ...
    worker._transform_runner = _TransformRunner()
    worker._engine_device = torch.device("cpu")
    worker._num_images = 2
    ...
    assert request.pixel_values.dtype is torch.float32
    assert request.step_noise.shape == (batch_size, 3, 10, 32)
