...
class PhyAIWorker(MultiStepRolloutWorker):
    ...
    def init_worker(self) -> None:
        ...
        config_name, input_transforms, output_transforms = (
            build_openpi_transforms_from_model_config(self.model_cfg)
        )
        transform_runner = OpenPITransformRunner(
            input_transforms,
            output_transforms,
        ...
        rollout_seed = self._phyai_cfg.get("seed")
    ...
    def _build_plugin_config(model_cfg: Any) -> PI05Config:
        ...
        return PI05Config(
            chunk_size=_resolve_model_action_horizon(model_cfg, openpi_cfg),
            max_action_dim=int(
    ...
    def _build_request(
        ...
        repacked = self._transform_runner.repack_env_obs(env_obs)
        transformed = self._transform_runner.input_transform(repacked)
        ...
        processed = self._transform_runner.observation_to_device(transformed)
    ...
    def _build_rollout_plan(self, request: PI05Request) -> _PI05SamplingPlan:
        """Resolve RLinf sampling policy into a concrete execution plan."""
        assert self._engine_device is not None
        openpi_cfg = self.model_cfg.get("openpi", {})
        ...
        sample_shape = (batch_size, action_horizon, model_action_dim)
        noise = torch.randn(
            sample_shape,
            dtype=torch.float32,
            device=self._engine_device,
    ...
    def _reduce_rollout_outputs(
        ...
        noise = plan.request.noise
        assert noise is not None
        batch_size, action_horizon, model_action_dim = noise.shape
        ...
        if plan.joint_logprob:
            initial = cls._initial_logprob(
                noise[:, : plan.action_chunk, : plan.action_dim]
            )
    ...
    def predict(
        ...
        if self._engine is None or self._transform_runner is None:
        ...
        actions = self._transform_runner.output_transform(
            {"actions": model_actions, "state": processed.state}
        )["actions"]
